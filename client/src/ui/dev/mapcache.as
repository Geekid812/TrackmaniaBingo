...
namespace UIDevMapCache {
    ...
    const string MAP_REQUEST_TMX_URL = "https://trackmania.exchange/mapsearch2/search?api=on&random=1&maptype=TM_Race";
    ...
    void RenderCacheTable() {
        if (UI::BeginTable("bingodevmapcache", COLUMN_HEADERS.Length)) {
            for (uint i = 0; i < COLUMN_HEADERS.Length; i++) {
                ...
                UI::TableSetupColumn(COLUMN_HEADERS[i], isTmxColumn ? UI::TableColumnFlags::WidthFixed : UI::TableColumnFlags::None, isTmxColumn ? TMX_COLUMN_WIDTH : 0.);
            }
            UI::TableHeadersRow();
    ...
    void LoadTMXMapsCoroutine(int amount) {
        ...
        while (amount > 0) {
            trace("[UIDevMapCache::LoadTMXMapsCoroutine] Fetching new map... (" + amount + " remaining)");
            ...
            if (body.HasKey("StyleName") && body["StyleName"].GetType() != Json::Type::Null) map.style = body["StyleName"];
            if (body.HasKey("Tags") && body["Tags"].GetType() != Json::Type::Null) map.tags = body["Tags"];

            MapCache.InsertLast(map);
        }

        print("[UIDevMapCache::LoadTMXMapsCoroutine] Loaded " + originalAmount + " new maps in local cache.");
        PersistantStorage::SaveDevMapCache();
    }
