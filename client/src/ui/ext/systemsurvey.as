...
namespace UISystemSurvey {
    ...
    void RenderConsentView() {
        UI::TextWrapped(
            "Hello! We are running a short automated survey to understand what system "
            "configuration Bingo players are using, similar to the Steam hardware survey.\n\nThe "
            "plugin will gather some basic information which you can choose to send to the "
            "developer of Trackmania Bingo for consideration in the development of future updates "
            "to the plugin. Do you want to participate?");
    }
    ...
    void RenderSummaryView() {
        ...
        UI::TextWrapped("Here is a summary of the information gathered about your setup. You can "
                        "review it and continue if you accept to share it with the developer:");
    ...
    void RenderAcceptedView() {
        UI::TextWrapped(
            "Thanks for participating!\n\nIf you are interested in hearing about development news "
            "and what is being done, you can always join the Discord server.\n\nHave fun!");
    }

    void RenderCancelledView() {
        UI::TextWrapped(
            "The survey was cancelled and you won't be asked to participate again. If you find any "
            "issues while using the plugin, please make sure to report them in the Discord server "
            "or on GitHub so they can be fixed as soon as possible!");
    }
    ...
    void SurveyChoiceButtons() {
        ...
        bool canShowDenyButton =
            State == SurveyViewState::Consent || State == SurveyViewState::Summary;
        ...
        float yesBtnWidth = Draw::MeasureString(acceptButtonText).x +
                            UI::GetStyleVarVec2(UI::StyleVar::FramePadding).x;
        float gapWidth = Math::Max(
            availableSpace.x - yesBtnWidth - UI::GetStyleVarVec2(UI::StyleVar::ItemSpacing).x, 0.0);
